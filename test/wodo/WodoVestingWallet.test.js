...
const WodoVestingWallet = artifacts.require('WodoVestingWallet');

const { shouldBehaveLikeVesting } = require('./WodoVestingWallet.behavior');
...
contract('WodoVestingWallet', function (accounts) {
  ...
  beforeEach(async function () {
    ...
    this.mock = await WodoVestingWallet.new(beneficiary, this.start, duration);
  });

  it('rejects zero address for beneficiary', async function () {
    await expectRevert(
      WodoVestingWallet.new(constants.ZERO_ADDRESS, this.start, duration),
